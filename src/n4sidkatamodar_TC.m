% Algorithm for the modified N4SID method, utilizing input data u, output y, and k as the order of Hankel blocks.
% By Alexander Robles, 2017.
% Modifications by Bob Xiaohai Hu, UW ME, 2023.
function [A,B,C,D,xf,n,Uf,Yf,U,Y] = n4sidkatamodar_TC(u,y,k)

[l,ny] = size(y);if (ny < l);y = y';[l,ny] = size(y);end
[m,nu] = size(u);if (nu < m);u = u';[m,nu] = size(u);end

% Determining the number of columns for the Hankel matrices.
N = ny-2*k+1;
% N = 853;

%Constructing the data matrices, Hankel block.
% H = blkhank(y,i,j)
% 
% Description:
